...
function navigateTo(pageId) {
  ...
  if (pageId === 'actions') renderAction('high');
...
function answerChoice(btn, isCorrect, feedbackId) {
  ...
  if (isCorrect) {
    ...
    feedback.innerHTML = '<strong>Пока нет.</strong> Сначала сопоставь ITPH с тем, что происходит в смене и опасных зонах, и выбери действие по причине.';
...
function showPattern(btn, type) {
  ...
  feedback.innerHTML = type === 'low'
    ? '<strong>18 мая:</strong> нагрузка была ниже цели, негативных отзывов не было. Это повод проверить эффективность расстановки.'
    : '<strong>20 мая:</strong> шесть часов перегрузки по ITPH совпали с 16 негативными отзывами. Вероятный риск качества заметен, но точную причину всё равно нужно подтвердить наблюдением за процессом.';
}
...
const ACTIONS = {
  high: {
    ...
    steps: [
      'Найди конкретное узкое место по составу заказов и движению очереди.',
      'Усиль станцию опытным сотрудником по SEEF или перестрой расстановку.',
      'Меняй людей на тяжёлых станциях и дай дополнительный отдых после пика, если это возможно.',
      'Если сотрудников не хватает, позови кого-то выйти пораньше и сообщи директору о причине.',
    ],
  },
  low: {
    ...
    steps: [
      'Убедись, что низкий ITPH связан с потоком, а не с медленной работой или сбоем.',
      'Сохрани минимально нужную готовность опасных зон.',
      'Перенаправь свободное время на подготовку, чистоту, обучение или перерывы.',
      'Верни сотрудника на станцию, как только поток и ITPH начнут расти.',
    ],
  },
  quality: {
    title: 'Проблема не в нагрузке',
    tag: 'Ищи процесс и стандарты',
    steps: [
      'Не объясняй жалобы цифрой: нормальный ITPH не гарантирует качество.',
      'Понаблюдай за приготовлением, сборкой, выдачей заказа и общением с Гостем.',
      'Найди конкретное отклонение от процесса или стандарта.',
      'Дай точечную обратную связь и проверь изменение на следующей контрольной точке.',
    ],
  },
};
