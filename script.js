...
function answerChoice(btn, isCorrect, feedbackId) {
  ...
  if (isCorrect) {
    ...
    feedback.innerHTML = '<strong>Пока нет.</strong> Сначала сопоставь ITPH с тем, что происходит на смене: скоростью обслуживания, работой станций и сотрудников. Затем выбери действие по причине.';
...
function showPattern(btn, type) {
  ...
  feedback.innerHTML = type === 'low'
    ...
    : '<strong>20 мая:</strong> ITPH был выше цели шесть часов подряд и совпал с 16 негативными отзывами. Проверь скорость обслуживания, работу станций и конкретные отзывы, чтобы подтвердить причину.';
}
...
const ACTIONS = {
  high: {
    ...
    steps: [
      ...
      'Встань в зону наблюдения и определи западающую станцию по скорости обслуживания и работе сотрудников.',
      'Пересмотри расстановку: усили западающую станцию опытным сотрудником или поменяй сотрудников местами.',
      'При длительной перегрузке попроси сотрудника выйти раньше или выйти в свой выходной. После пика наведи чистоту и порядок; перерывы — только по расписанию.',
    ],
  },
  low: {
    ...
    steps: [
      'Проверь часовую динамику: это первый час ниже цели или отклонение повторяется.',
      'Убедись, что причина не в медленной работе станции или техническом сбое.',
      'Не переставляй сотрудников только ради цифры: сама перестановка ITPH не изменит.',
      'При повторении отпусти сотрудника домой, выведи кого-то позже или перенаправь в другой ресторан — без ущерба следующему шифт-тренеру.',
    ],
  },
  quality: {
    ...
    steps: [
      ...
      'Проверь технический сбой: например, заказов много, а в системе они не отображаются.',
      'При рассинхроне уточни у соседнего ресторана, есть ли проблема, и сделай заявку в поддержку. Затем дай точечную обратную связь сотруднику.',
    ],
  },
};
